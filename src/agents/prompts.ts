...
【本タスクの種別】
本タスクは【調査・検討・設計タスク（※コード実装は行わない）】です。
【役割】
チケットの背景・論点に基づき、技術調査、フィジビリティスタディ、比較検討を行い、調査報告書および設計書を作成してください。
【実施事項】
1. チケットの論点・要件を整理し、必要な調査・検証・技術選定を行う
2. 成果物として、マークダウン形式の調査報告書・設計書（例: \`docs/investigation_report.md\` または \`docs/detailed_design.md\` 等）を作成・更新する
3. 変更内容を Git コミットする
...
作業が完了したら、調査結果・設計内容の要約と「次は curator による調査・設計レビューです」と報告してください。`;
      }
...
本タスクは【調査・検討・設計タスク（※コード実装は行わない）】です。
【役割】
directorが作成した調査報告書・設計書の妥当性、論点の網羅性、技術選定の根拠を客観的にレビューしてください。
【実施事項】
1. 調査結果や設計方針の妥当性、抜け漏れ、実現可能性を精査する
2. 懸念点や不足、修正すべき箇所があれば、具体的に指摘し「directorへ差し戻し」と明記する
3. 根本的な方針決定など人間の判断が必要な場合は、「【人間への確認依頼】」または「CONFIRM_HUMAN」と明記してエスカレーションしてください（※エスカレーションが不要な場合は、これらのキーワードを出力文中に含めないでください）。
4. 問題がなければ「承認（LGTM）」し、「次は全工程完了（調査完了）です」と報告してください（※本タスクは調査タスクのため、artistによる実装へは進みません）。`;
      }
