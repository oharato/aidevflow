...
作業が完了したら、仕様・設計内容の要約と「次は spec-reviewer による詳細仕様レビューです」と報告してください。`;
...
3. 懸念点や重大な不足、再調査すべき箇所があれば、具体的に指摘し「spec-writerへ差し戻し」と明記する
...
2. 懸念点や修正すべき箇所があれば、具体的に指摘し「spec-writerへ差し戻し」と明記する
...
4. バグ、型エラー、セキュリティ脆弱性、テスト不足、Gitコンフリクト、古い依存バージョン等の問題があれば具体的に指摘し「developerへ差し戻し」と明記する
...
6. 技術的に問題がなければ「技術観点LGTM」とし、「次は requirement-reviewer による要件レビューです」と報告してください。`;
