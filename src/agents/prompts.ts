...
2. 【要件の過不足・波及分析 (Aに対するBの検討)】:
   - チケットに明記された機能要件（A）に対して、システムを安全・確実に稼働させるために不可欠な関連要件（B）を自問自答し、過不足を洗い出す:
     ...
     - **非機能要件・環境制約**: 「A（新UIやAPI）に対し、同一LAN（0.0.0.0バインド/Vite Proxy）からの接続やDockerボリューム権限、バージョン固定規約をどう満たすか？」
     - **既存アーキテクチャ資産との整合性**: 「過去チケットで作成された共通クライアント（Ollama互換クライアント等）や宣言的マイグレーション（Atlas一本化）の規約を遵守しているか？独自に車輪の再発明をしていないか？」
