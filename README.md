...
| :--- | :--- | :--- | :--- |
| **通常モード** | 通常のチケット作成 | `architect` → `tech-lead` → `developer` → `code-reviewer` → `requirement-reviewer` (5段階) | 新機能開発や大規模改修向け |
| ⚡ **Fast モード** | 件名に `[fast]` を付与、または本文に `mode: fast` | `developer` → `code-reviewer` (2段階で即PR作成・完了) | 既知のバグ修正・文言修正・軽微な変更用（クォータ消費を約 60% 削減） |
| 🔍 **調査モード** | 件名に `[調査]` を付与、または種別・カテゴリーを「調査」 | `architect` → `tech-lead` (2段階で調査ドキュメント作成・完了) | 技術選定・比較検証・アーキテクチャ設計用（コード実装をスキップして完了） |
