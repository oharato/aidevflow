...
件名に `[AI]`（必要に応じて `[fast]` や `[調査]`）を付け、課題の「詳細」に対象リポジトリ（単数または複数）と要件を記載します：

```markdown
件名: [AI] ユーザー一覧画面の実装
（または Fastモード: [AI] [fast] ログイン文言の修正、調査モード: [AI] [調査] 認証基盤の比較検証）
...
> 💡 **チケット作成テンプレート**:
> Backlogのプラン制限で「課題テンプレート」機能が使えない場合でも、定義漏れや手戻りを防ぐコピペ用テンプレート **[docs/TICKET_TEMPLATE.md](docs/TICKET_TEMPLATE.md)** を用意しています。対象リポジトリ、機能要件（0件時・異常系）、非機能要件（LANアクセス、共通モジュール利用、バージョン固定）、検証用seedデータ、完了定義（DoD）のチェックリストを網羅しています。
>
> 💡 **ドラフト保護**: チケット作成直後のステータスは「未対応」のため、AI はまだ起動しません。下書きがすべて完了するまで安全に編集できます。下書き完了後にステータスを **「処理中」** に変更して開始してください。
