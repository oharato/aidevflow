...
> [!TIP]
> **⚡ LLMモデルと推論エフォートの最適化（クォータ節約）**:
> デフォルトではクォータ消費を抑えつつ高速に動作する `gemini-3.8-flash-high` およびレビュー用の `gemini-3.8-flash-medium`、推論エフォート `low` が設定されています：
> ```bash
> AGY_MODEL=gemini-3.8-flash-high
> AGY_REVIEW_MODEL=gemini-3.8-flash-medium
> AGY_EFFORT=low
> ```
...
登録される状態：
- **詳細設計中** (`#3b9dbd` 水色) - 担当: Architect (要件から詳細設計書を作成)
- **設計レビュー中** (`#868cb7` 青紫) - 担当: Tech-Lead (詳細設計書の客観的レビュー)
- **実装中** (`#eda62a` オレンジ) - 担当: Developer (コード実装、テスト、最新安定版の依存選定)
- **技術レビュー中** (`#b0be3c` 黄緑) - 担当: Code-Reviewer (技術レビュー: 型/規約/バグ/言語・ライブラリの最新性点検)
- **要件レビュー中** (`#e07b9a` ピンク) - 担当: QA (チケット要件の充足度レビュー)
...
「コード実装ではなく、技術選定、比較検証、アーキテクチャ設計、スパイク（Spike）を行いたい」という場合、**設計（Architect）と設計レビュー（Tech-Lead）のみで完了する「調査モード」** を利用できます。
...
#### 🔄 調査パイプラインの流れ
1. **Architect（調査・検討・設計・リポジトリ修正）**:
   - チケットの論点や要件を整理し、技術検証や比較検討を実施。
   - **指示に応じたリポジトリの修正**: 「リポジトリにドキュメント残して」「READMEに追記して」「docs/に設計書を作成して」等の指示がある場合、リポジトリ内のドキュメント（`docs/investigation_report.md`、`docs/detailed_design.md`、`README.md` 等）や検証コードを直接作成・修正してコミット。
   - **未コミット成果物の自動保護**: エージェントがコミットコマンドを実行し忘れた場合でも、デーモンが未コミット差分を検知して自動コミット（`docs(architect): ...`）するため成果物が消失しません。
   - 完了すると自動的に `[調査レビュー中]` へ遷移。
2. **Tech-Lead（調査レビュー）**:
   - 調査報告書やリポジトリの変更差分（`git diff`）の妥当性・論点の網羅性を客観的にレビュー。
   - 軽微な表記揺れ等は自らリポジトリファイルを修正・コミット可能。
   - 不足があれば `architect` へ差し戻し（`[調査中]` へ戻る）。
   - 問題がなければ **承認（LGTM）** し、**コード実装（Developer）へは進まずに全工程完了！**
3. **完了・レビュー報告 & GitHub PR 作成**:
   - リポジトリに変更がある場合は自動的に GitHub へ push され、**Pull Request が自動作成** されます。
   - ステータスが **「処理済み」**（件名: `[調査完了]`）に更新され、調査報告書の要約と PR リンクが Backlog コメントに投稿されます。
   - 人間が PR を確認・マージし、Backlog チケットを「完了」にしてクローズします。
   - 追加調査や修正指示がある場合はコメントしてステータスを「処理中」に戻すと、自動的に `architect` が再起動して再調査・ドキュメント更新を行います。

### 7. Fast モードの実行方法（軽量2段階パイプライン）

「既知の軽微なバグ修正」「文言やスタイルの変更」「小規模なリファクタリング」など、詳細設計フェーズ（Architect/Tech-Lead）を必要としないタスクでは、**実装（Developer）と統合レビュー（Code-Reviewer）の2フェーズのみで即座にPR作成・完了する「Fast モード」** を利用できます。

#### 💡 Fast モードとして実行する方法（以下のいずれか1つでOK）
- **件名に `[fast]` または `【fast】` を付ける**: 例: `[fast] ヘッダーロゴの余白修正`
- **本文に指定する**: チケット本文に `モード: fast` または `mode: fast` と記載

#### 🔄 Fast パイプラインの流れ
1. **Developer（実装・単体テスト・PR作成）**:
   - チケット要件から直接コードを修正し、テストを実行・コミット。
   - GitHub へのブランチ push と Pull Request 作成を自動実行。
   - 完了すると自動的に `[技術レビュー中]` へ遷移。
2. **Code-Reviewer（技術＆要件統合レビュー）**:
   - コードの品質・型・規約チェックに加え、元のチケット要件を満たしているかを一括して厳格にレビュー。
   - 問題があれば `developer` へ差し戻し（`[実装中]` へ戻る）。
   - 問題がなければ **承認（LGTM）** し、そのまま **全工程完了！**（要件レビューフェーズをスキップ）。
3. **完了・レビュー報告**:
   - ステータスが **「処理済み」**（件名: `[要件レビュー完了]`）に更新され、PRリンク付きの完了コメントが投稿されます。
...
> [!TIP]
> **🔒 二重起動防止機能（排他プロセスロック）**:
> `aidevflow` は、フォアグラウンド・バックグラウンド・systemd などの起動方法を問わず、起動時にルートディレクトリの `.aidevflow.lock` を介して**厳格な排他制御（PID 生死確認付き）**を行います。
> - 既に別プロセスが稼働している場合、起動を即座にブロックして終了します（Git ワークツリーの破壊やチケットステータスの上書き・巻き戻しを完全に防止）。
> - クラッシュ等でロックファイルだけが取り残された場合（Stale lock）は、PID のプロセス生存確認を行い、自動的に古いロックを回収して安全に起動します。
> - `pnpm run status` や `pnpm run stop` は、フォアグラウンド・バックグラウンドいずれの起動プロセスも自動検知して確認・停止できます。
...
stateDiagram-v2
    ...
    詳細設計中 --> 確認待ち: 判断不能な論点発生 / クォータ枯渇
    実装中 --> 確認待ち: 判断不能な論点発生 / クォータ枯渇
...
3. **その後のAIの動き**:
   - デーモンが人間の指示コメントを検知し、自動的に `developer`（実装エージェント）を起動します。
   - 既存ブランチ（例: `STUDY-3`）の作業 Worktree でコードを修正し、コミットを作成して **既存の GitHub PR に追記 push** します。
   - その後、自動で `code-reviewer`（技術レビュー）→ `qa`（要件レビュー）が走り、問題なければ再度「処理済み」として報告されます。
...
PR 作成時（Developer 完了時）および全工程完了時（QA 承認時）に、Backlog のコメント欄へ**対象リポジトリの構成（Docker Compose, package.json, requirements.txt 等）を自動判別した手元での動作確認手順**が自動投稿されます。
...
### ③ エージェント異常終了・LLMクォータ上限（Quota Reached）時の安全停止
...
- 即座にステータスを **「確認待ち」**（件名プレフィックスモードでは `[確認待ち]` かつ「未対応」）に変更し、エラー詳細と復帰手順を Backlog に自動報告して一時停止します。
- 再開時は同一ロールが保持されるため、クォータ回復後やエラー解消後にステータスを「処理中」に戻すだけで、そのフェーズから安全に自動再開されます。

### ④ 人間による再開フロー (Resume)
1. 人間がチケットのコメントで指示・方針を回答します（クォータ制限時はリセットを待つか、環境変数・キー設定を確認）。
2. チケットのステータスを **「処理中」**（カスタム状態利用時は「詳細設計中」または「実装中」）に戻します。
3. デーモンが「確認待ち」からの復帰を自動検知し、**差し戻しカウンターをゼロにリセット** して自律作業を再開します。
