...
1. **Director（調査・検討・設計・リポジトリ修正）**:
   ...
   - **未コミット成果物の自動保護**: エージェントがコミットコマンドを実行し忘れた場合でも、デーモンが未コミット差分を検知して自動コミット（`docs(director): ...`）するため成果物が消失しません。
...
3. **完了・レビュー報告 & GitHub PR 作成**:
   ...
   - 追加調査や修正指示がある場合はコメントしてステータスを「処理中」に戻すと、自動的に `director` が再起動して再調査・ドキュメント更新を行います。

### 7. Fast モードの実行方法（軽量2段階パイプライン）

「既知の軽微なバグ修正」「文言やスタイルの変更」「小規模なリファクタリング」など、詳細設計フェーズ（Director/Curator）を必要としないタスクでは、**実装（Artist）と統合レビュー（Critic）の2フェーズのみで即座にPR作成・完了する「Fast モード」** を利用できます。
...
stateDiagram-v2
    ...
    設計レビュー中 --> 調査完了: curator承認 (調査タスク時: コード実装をスキップして完了)
    設計レビュー中 --> 詳細設計中: curator差し戻し (リトライ < 上限)

    実装中 --> 技術レビュー中: artist実装・コミット完了\n★ 修正された各リポジトリから GitHub PR 自動作成

    技術レビュー中 --> 要件レビュー中: critic承認 (通常モード LGTM)
    技術レビュー中 --> 処理済み: critic承認 (Fast モード LGTM: 直接完了)
