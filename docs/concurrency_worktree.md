...
- 🔀 **[並行開発 & Git Worktree 仕様書](concurrency_worktree.md)** (本ドキュメント)
...
2. Backlog のステータスが **「完了」**（ライフサイクル `closed` = 人間がクローズ済み）であること。「未対応」「処理中」「確認待ち」「要件レビュー中」、および **「処理済み」・`[要件レビュー完了]`（ライフサイクル `completed` = AI 全工程完了・人間の PR レビュー待ち）は絶対に削除されません**。
3. 関連リポジトリの GitHub プルリクエストが **「CLOSED」「MERGED」または PR なし（調査タスク等）** であること。PR が `OPEN` の間は人間がマージ作業中として保護し、`gh` の失敗などで状態が **不明（UNKNOWN）の場合も判断不能として保護** します。
4. `DRY_RUN=true` の場合は、対象を判定してログに出すだけで削除・停止コマンドは実行しません。

孤児 Docker Compose プロジェクトの掃除も、**このデーモンの `~/aidevflow/worktrees/` 配下で起動されたものだけ**を対象とし、チケットの照会に失敗した場合は保護側に倒します（ホスト上の無関係なスタックや、別ユーザーの個人用デーモンの worktree には触れません）。
