...
stateDiagram-v2
    ...
    %% 人間の設計承認ゲート (REQUIRE_HUMAN_SPEC_APPROVAL=true時)
    設計レビュー中 --> 設計承認待ち: spec-reviewer承認 (LGTM) [承認ゲート有効時]
    設計承認待ち --> 実装中: 人間が設計書を確認し承認\n(ステータス「処理中」に変更)
    
    %% 承認ゲート無効時
    設計レビュー中 --> 実装中: spec-reviewer承認 (LGTM) [承認ゲート無効時]
...
3. **要件の過不足・波及分析 (Aに対するBの検討)**:
   - `spec-writer`（設計策定）および `spec-reviewer`（設計レビュー）において、「Aという要件に対し、関連してBという要件はどうするか？」の自律的な深掘りと検証を義務付けています。
   - **検討観点**:
     - **エッジケース・異常系**: 検索ヒット0件時の挙動（ハルシネーション防止、空状態UI）、APIエラー時のフォールバック。
     - **非機能要件**: LANアクセス対応（`0.0.0.0` バインド、Vite Proxy）、Dockerボリューム永続化と権限分離、依存ライブラリのバージョン固定。
     - **既存資産との整合性**: 過去チケットで策定した共通アーキテクチャ（共通クライアント、DBスキーマ管理ツール等）の再利用。
     - **検証用データ**: 動作確認に必要なシードデータ（DB初期投入データ、リクエスト例）の定義。
   - 判断に複数の有力な選択肢が存在する場合や、ビジネス要件に関わる判断が必要な場合は、AIが自己完結して独断で進めず、`CONFIRM_HUMAN` でBacklog上で人間に確認を求めます。

4. **人間の設計承認ゲート (`REQUIRE_HUMAN_SPEC_APPROVAL`)**:
   - 設計レビュー（`spec-reviewer`）完了後、実装（`developer`）へ進む前に自動的に **`[設計承認待ち]`（ステータス: 未対応）** で一時停止します。
   - **目的**: 「設計方針の齟齬による実装・レビューの大幅手戻り（STUDY-5/6のようなケース）」をゼロにすること。設計段階で人間が1分レビューすることで、数時間の実装・レビューの手戻りを未然に防ぎます。
   - **再開手順**:
     - 人間がリポジトリ内の詳細設計書（`docs/detailed_design.md` や PR差分）を確認。
     - 問題がなければステータスを **「処理中」** に変更するだけで、デーモンが検知して自動的に `developer` が実装を開始。
     - 修正や方針変更がある場合は、チケットに指示をコメントして「処理中」に変更すれば、AIが修正を反映します。
   - `.env` で `REQUIRE_HUMAN_SPEC_APPROVAL=true` に設定することで有効化されます（デフォルトは無効で一気通貫）。

5. **エージェント異常終了・LLMクォータ上限（Quota Reached）時の安全停止**:
...
### 4. クォータ枯渇ロック・Backlogポーリング休止・自動回復プローブ
- **ローカルロックファイル (`.aidevflow.quota.lock`)**:
  - クォータ上限（Quota reached / 429）を検知すると直ちにローカルにロックファイルを作成し、推定リセット時刻を記録。
- **Backlog ポーリング休止**:
  - クォータ枯渇中は Backlog へのポーリング（`getIssues`）を完全停止し、無駄な API コールや他チケットの巻き込み停止を防止。
- **自動回復プローブ & 全自動再開 (Auto-Resume)**:
  - リセット時刻が到来すると超軽量プローブ（`agy -p "ping"`）を実行して回復を検証。
  - 回復確認時にロックを自動解除し、中断チケットを自動で再開（手動操作不要）。
