...
- **Director（調査・設計）**:
  ...
  - マークダウン形式の成果物（`docs/investigation_report.md` または `docs/detailed_design.md`）を作成し、Git コミット。
- **Curator（調査レビュー）**:
  - 調査結果や設計方針の妥当性、論点の網羅性を客観的にレビュー。
  - 不足があれば Director へ差し戻し。
  - 問題がなければ **「承認（LGTM）」** とし、**Artist（実装）へは進まず全工程完了（調査完了）** と判定。
- **Backlog ステータス更新**:
  - 件名: `[調査完了] チケット名`
  - ステータス: **「処理済み」**（カスタム状態利用時は「完了」）
  - コメント: 調査報告書の要約および人間向けの対応手順を自動投稿。
- **人間による追加指示フロー**:
  - 人間がレビュー後にチケットコメントに追加指示を書き、ステータスを **「処理中」** に戻すと、自動的に **Director（再調査・設計修正）** が再起動します。
